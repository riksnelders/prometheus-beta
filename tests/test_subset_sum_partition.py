...
def test_basic_cases():
    ...
    assert count_equal_sum_partitions([1, 2, 3, 4, 5, 7]) == 2
    assert count_equal_sum_partitions([1, 1, 1, 1]) == 6
...
def test_zero_sum():
    ...
    assert count_equal_sum_partitions([0, 0, 0, 0]) == 6

def test_mixed_numbers():
    # Test with mixed positive and negative numbers
    assert count_equal_sum_partitions([1, -1, 2, -2]) == 6